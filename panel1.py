...
# Warna premium
R = "\x1b[38;5;196m"  # red neon
B = "\x1b[38;5;87m"   # blue neon
W = "\x1b[1;37m"      # white
G = "\x1b[38;5;46m"   # green neon
X = "\x1b[0m"

def slow(text, delay=0.004):
    for c in text:
        sys.stdout.write(c)
        sys.stdout.flush()
        time.sleep(delay)
...
# Banner lines
lines = [
    f"{B}╔═══════════════ ◆ {R}PARADISE PRIVATE TOOLS{B} ◆ ═══════════════╗",
    f"{B}║{X}{center(f'{G}VERSION   : {W}5.0 FINAL BUILD').strip()}{B}║",
    f"{B}║{X}{center(f'{G}AUTHOR    : {W}PARADISE CYBER ENGINE').strip()}{B}║",
    f"{B}║{X}{center(f'{G}GITHUB    : {W}github.com/PARADISE-TEAM').strip()}{B}║",
    f"{B}║{X}{center(f'{G}STATUS    : {W}ACTIVE 🔥  | MULTI-TASK MODE ⚡').strip()}{B}║",
    f"{B}║{X}{center(f'{G}SERVERS   : {W}[SG] [US] [JP] [ID] 🌍').strip()}{B}║",
    f"{B}║{X}{center(f'{G}SECURITY  : {R}ENCRYPTED 🔒 | ANTI-DETECT 🛡️').strip()}{B}║",
    f"{B}║{X}{center(f'{G}UPDATED   : {W}14-NOV-2025').strip()}{B}║",
    f"{B}╚" + "═" * 59 + "╝"
]
