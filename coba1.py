...
def prds_spinner():
    ...
    while not stop_spinner:
        ...
        sys.stdout.write(f"\r{color}[PRDS⚡]{next(dots)}\033[0m ")
        sys.stdout.flush()
        i += 1
        time.sleep(0.08)
...
def ____banner____():
    ...
    def gradient(text):
        colors = [
            ...
            (255,80,0),   # orange
            (255,200,0),  # yellow
            (0,255,120),  # green neon
            (0,200,255),  # cyan neon
            (120,0,255),  # purple neon
        ]
        out = ""
    ...
    # Fade line (gelap -> terang)
    def fade_in(text):
        for i in range(0, 255, 25):
            sys.stdout.write(f"\033[38;2;{i};{i};{i}m{ text }\r")
            sys.stdout.flush()
            time.sleep(0.015)
        sys.stdout.write(RESET + text + "\n")
    ...
    title = gradient("◆ PARADISE PRIVATE TOOLS ◆")
    box_top = gradient("╔" + "═" * 58 + "╗")
    box_bot = gradient("╚" + "═" * 58 + "╝")

    info = [
        "VERSION    : 6.0 FINAL ELITE BUILD",
        "AUTHOR     : PARADISE CYBER ENGINE",
        "GITHUB     : github.com/PARADISE-TEAM",
        "STATUS     : ACTIVE ⚡  | MULTI-TASK ENABLED",
        "SERVERS    : SG | US | JP | ID  🌍",
        "SECURITY   : QUANTUM-ENCRYPT 🔒 | ANTI-DETECT 🛡️",
        "UPDATED    : 17-NOV-2025",
    ]

    # ======= ANIMASI MULAI =======
    fade_in(center(box_top))
    time.sleep(0.05)

    # Pulse highlight title
    for i in range(3):
        sys.stdout.write(center(rgb(255,255,255) + title + RESET) + "\r")
        sys.stdout.flush()
        time.sleep(0.15)
        sys.stdout.write(center(rgb(120,120,120) + title + RESET) + "\r")
        sys.stdout.flush()
        time.sleep(0.15)
    type_fx(center(title))

    # Isi kotak
    for line in info:
        type_fx(center("║ " + gradient(line) + " " * 2 + "║"), 0.001)

    fade_in(center(box_bot))
    print(RESET)
...
def login_1(uid):
    ...
    try:
        # ============================
        # START SPINNER PRDS RAINBOW
        # ============================
        global stop_spinner
        stop_spinner = False
        spin_thread = threading.Thread(target=prds_spinner)
        spin_thread.start()
        
        # ====== proses tampilan lama ======
        sys.stdout.write(
    ...
    "User-Agent": str(random.choice([
        ...
        "x-fb-connection-token": "d29d67d37eca387482a8a5b740f84f62"
            ...
            if 'session_key' in res:
                ...
                try:
                    import winsound
                    winsound.Beep(1500, 200)  # nada lebih tinggi dari CP
                    winsound.Beep(2000, 150)  # nada kedua premium
                except:
                    print("\a")  # fallback Linux/Termux
                ...
                def R(i): return rb[i % len(rb)]
                def box(t, w=33): return t + (" " * (w - len(t)))

                year = str(creationyear(uid))
                ...
                print(R(2) + "├" + "─" * 45 + "┤")
                print(R(3) + "│ UID     : " + box(uid) + "│")
                print(R(4) + "│ PASS    : " + box(pw) + "│")
                print(R(5) + "│ YEAR    : " + box(year) + "│")

                st_color = "\033[92m" if status == "UREP" else "\033[91m"
                print(R(6) + "│ STATUS  : " + st_color + box(status) + "\033[0m│")

                print(R(7) + "│ USER-AG : " + box(ua[:33]) + "│")
                print(R(8) + "├" + "─" * 45 + "┤")
            ...
            ## ===================== LOGIN CP (CHECKPOINT) =====================
                ...
                try:
                    import winsound
                    winsound.Beep(1000, 300)  # frekuensi 1000Hz, durasi 300ms
                except:
                    print("\a")  # fallback Linux/Termux
